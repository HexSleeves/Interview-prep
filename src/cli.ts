...
function findProblemById(id: string): Problem<unknown, unknown> | undefined {
	return allProblems.find((p) => p.id === id) as Problem<unknown, unknown> | undefined;
}
...
function runDomain(domain: string): void {
	...
	const result = runSuite(problems as Problem<unknown, unknown>[]);
...
function runAll(): void {
	...
	const result = runSuite(allProblems as Problem<unknown, unknown>[]);
	console.log(formatSuiteResult(result));
